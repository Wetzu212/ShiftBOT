...
client.on("ready", () => {
  ...
  client.user.setGame('with my maker~ | +help');
});
...
client.on("guildDelete", guild => {
  ...
  client.user.setGame('${client.guilds.size} guilds | +help');
});
...
client.on("message", async message => {
 ...
 if(command === "ban") {
   // Most of this command is identical to kick, except that here we'll only let admins do it.
    ...
    let member = message.mentions.members.first();
    if(!member)
      return message.reply("Please mention a valid member of this server. Dummy! Dummy");
    if(!member.bannable)
 ...
 if (command === 'help') {
    message.reply(' `These are my commands!`\n`+ban`: bans a person\n`+kick`: kicks a person\n`+say`: says something\n`+ping`: sufficiently clear\n`+purge`: purges messages up to 100 ');
}
...
client.login('Mzk1MzgzNzIyNzU4NjM1NTIw.DTUAQg.g9X1SldxuilO6FJTgfaMyLuSMX4');